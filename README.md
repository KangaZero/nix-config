...
├── CHANGE_LOG.md
├── CVE_REPORT_X86.md                 # Generated by the weekly cron workflow
...
|---|---|---|
| `cve-linux` | `ubuntu-latest`, in the `nixos/nix` container | Builds `nixosConfigurations.nixos`, runs vulnix, uploads `CVE_REPORT_X86.md` as artifact. Nix comes from the image, so no installer action; `magic-nix-cache` is omitted because Actions Cache rate-limiting (429 → nix `HTTP 418`) broke this job through Aug 2026 |
...
| `commit` | `ubuntu-latest` | Downloads both artifacts, commits both reports in a single commit, dated `YYYY-MM-DD` (UTC). Rebases first, since the scans take 30-60 min and `main` can move underneath |
...
|---|---|---|
| Linux / NixOS WSL | `.#nixosConfigurations.nixos.config.system.build.toplevel` | [`CVE_REPORT_X86.md`](./CVE_REPORT_X86.md) |
