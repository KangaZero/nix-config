...
### Extra requirements without Nix

On this repo's hosts, several things arrive prebuilt from Nix. Using this config
standalone means supplying them yourself:

| Tool | Why it is needed off Nix |
| --- | --- |
| **`tree-sitter` CLI** + a **C compiler** | Nix normally installs 21 prebuilt grammars into `pack/hm/start/nvim-treesitter-grammars/` via `programs.neovim.plugins`. Without it you have only Neovim's 7 bundled parsers (`c`, `lua`, `markdown`, `markdown_inline`, `query`, `vim`, `vimdoc`), so run `:TSInstall <lang>` — which shells out to `tree-sitter build`, and fails without the CLI (`:checkhealth nvim-treesitter` reports it) |
| **`make`** + a **C compiler** | `telescope-fzf-native` ships a C sorter (`libfzf.so`) that Nix pre-builds. See the plugin table for how to let `vim.pack` fetch it instead |
| **Mason** (bundled) | installs the LSP servers and `stylua`. `lsp.lua` gates `ensure_installed` on `/etc/NIXOS`, so off Nix Mason does the work — this needs `node` on `PATH` for the node-based servers |

Two further differences, neither fatal:

- **`.luarc.json` is generated by Nix** (`neovim.nix` writes it with the Neovim
  runtime's Lua path). Without it `lua_ls` does not know the `vim` global and reports
  `Undefined global 'vim'` throughout. Copy one in or point `workspace.library` at your
  own Neovim runtime.
- **`tsc` is NixOS-only.** `lsp.lua` enables the TypeScript 7 server only when
  `/etc/NIXOS` exists, because only Nix pins TS >= 7; elsewhere it falls back to
  Mason-installed `vtsls`.
